...
3. **Wrangle**: handle data types, check missingness, outliers, and scaling needs; create derived features if justified (e.g., total acidity proxies).
4. **EDA**: compare red vs white distributions; analyze relationships between features and quality; identify multicollinearity.
5. **Modeling**: train baseline models (e.g., linear / logistic baselines) and improved models (e.g., tree-based methods). Evaluate with clear metrics and robust validation.
...
1. **Predictive question**:  
   *How accurately can we predict a wine’s quality score using its physicochemical properties?*  
   - Output: model performance (e.g., RMSE/MAE for regression; or accuracy/F1 if reframed as classification such as “high vs not-high quality”).

2. **Comparative question**:  
   *Do the drivers of quality differ between red and white wine?*  
   - Output: side-by-side comparisons of feature effects/importance; interpretation of differences (e.g., whether acidity or alcohol has stronger association with quality for one type).
...
3. **Practical insight question**:  
   *Which measurable factors provide the most actionable insight for quality improvement, and are there thresholds where quality tends to increase?*  
   - Output: interpretable findings (e.g., feature importance + plots showing relationships), with clear caveats that correlation ≠ causation.
...
- **Task J: Improved models + validation** (Owner: Member 2)  
  - Compare at least 2 model families (e.g., regularized linear vs tree-based); cross-validation; avoid leakage.
...
5. **Multicollinearity**: Some features may be correlated (e.g., acidity-related measures). Linear models may be unstable without regularization; we will check VIF/correlation and consider regularized models.
...
1. **Course expectations for “integration”**: Our integration uses shared schema (red + white) and concatenation with `wine_type`. If the course expects linking via external IDs (e.g., geography/time/product IDs), we may need to add a third dataset. We will confirm with the instructor/TA early.
...
3. **Ethics / responsible use framing**: We will clarify which course topics must be explicitly addressed (e.g., transparency, bias, reproducibility) and incorporate them into the write-up.
...
5. Submit the release URL in Canvas:
   - `https://github.com/<org-or-user>/<repo>/releases/tag/project-plan`